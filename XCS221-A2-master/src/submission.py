...
class ShortestPathProblem(SearchProblem):
    ...
    def successorsAndCosts(self, state: State) -> List[Tuple[str, State, float]]:
        ...
        for neighbor, distance in self.cityMap.distances[state.location].items():
            successors.append((neighbor, State(neighbor), distance))
        return successors
        # ### END CODE HERE ###
...
class WaypointsShortestPathProblem(SearchProblem):
    ...
    def isEnd(self, state: State) -> bool:
        ...
        return self.endTag in self.cityMap.tags[state.location] and \
           all(tag in state.memory for tag in self.waypointTags)
        # ### END CODE HERE ###
    ...
    # Iterate over all adjacent locations from the current location.
        for next_location in self.cityMap.distances[state.location]:
        # Calculate the cost of moving to the next location.
            cost = self.cityMap.distances[state.location][next_location]

        # Check if the next location has any of the waypoint tags.
            next_location_tags = self.cityMap.tags[next_location]
            if any(tag in next_location_tags for tag in self.waypointTags):
            # If the next location contains any waypoint tag, update the memory.
                visited_tags = state.memory.union(next_location_tags)
                next_state = State(location=next_location, memory=visited_tags)
                successors.append((next_location, next_state, cost))
            else:
            # If the next location doesn't contain any waypoint tag, keep the memory unchanged.
                next_state = State(location=next_location, memory=state.memory)
                successors.append((next_location, next_state, cost))
        return successors
        # ### END CODE HERE ###
...
class StraightLineHeuristic(Heuristic):
    ...
    def evaluate(self, state: State) -> float:
        ...
        loc1_coords = self.cityMap.geoLocations[state.location]
        loc2_coords = self.cityMap.geoLocations[end_location]
        return computeDistance(loc1_coords, loc2_coords)
        # ### END CODE HERE ###
