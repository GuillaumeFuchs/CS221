...
def formula2d():
    ...
    # ### START CODE HERE ###
    return Forall('$x', Forall('$y', Equiv(Grandmother('$x', '$y'), And(Female('$y'), Exists('$z', And(Parent('$x', '$z'), Parent('$z', '$y')))))))
    # ### END CODE HERE ###
...
def ints():
    ...
    # Law 5: Larger is a transitive property.
...
def createRule1():
    ...
    return GrammarRule('$Clause', ['every', '$Noun', '$Verb', 'some', '$Noun'],
        lambda args: Forall('$x', Exists('$y', And(Atom(args[1].title(), '$x', '$y'), Atom(args[2].title(), '$y'))))
    )
    # ### END CODE HERE ###
...
def createRule2():
    ...
    return GrammarRule('$Clause', ['there', 'is', 'some', '$Noun', 'that', 'every', '$Noun', '$Verb'],
        lambda args: Exists('$x',Forall('$y', Forall('$z', Implies(And(Atom(args[1].title(), '$y'), 
        Atom(args[2].title(), '$z')),Atom(args[0].title(), '$x')))))
    )
    # ### END CODE HERE ###
...
def createRule3():
    ...
    return GrammarRule('$Clause', ['if', 'a', '$Noun', '$Verb', 'a', '$Noun', 'then', 'the', 'former', '$Verb', 'the', 'latter'], 
        lambda args: Forall('$x', Forall('$y', Implies(AndList([Atom(args[0].title(), '$x'), 
        Atom(args[2].title(), '$y'), Atom(args[1].title(), '$x', '$y')]), Atom(args[3].title(), '$x', '$y'))))
    )
    # ### END CODE HERE ###
