...
def create_chain_csp(n):
    ...
    for i in range(1, n):
        csp.add_binary_factor(f"x{i}", f"x{i+1}", lambda x, y: x ^ y)
    # ### END CODE HERE ###
    return csp
...
def create_nqueens_csp(n=8):
    ...
    # ### START CODE HERE ###
    for i in range(n):
        csp.add_variable('Q{}'.format(i), range(n)) 

    for i in range(n):
        for j in range(i + 1, n):
            def conflict(row_i, row_j):
                return row_i != row_j  # Queens cannot be in the same row
            csp.add_binary_factor('Q{}'.format(i), 'Q{}'.format(j), conflict)

    for i in range(n):
        for j in range(i + 1, n):
            def diagonal_conflict(row_i, row_j):
                return abs(i - j) != abs(row_i - row_j)
            csp.add_binary_factor('Q{}'.format(i), 'Q{}'.format(j), diagonal_conflict)
...
class BacktrackingSearch:
    ...
    def get_unassigned_variable(self, assignment):
        ...
        if not self.mcv:
            ...
            ##pass
            # ### START CODE HERE ###
            vv = [(sum(1 for val in self.domains[var] if self.get_delta_weight(assignment, var, val) > 0), var)
                        for var in self.csp.variables if var not in assignment]
            # Modificar esta línea
            return min(vv, key=lambda x: x[0])[1] if vv else None
...
def get_sum_variable(csp, name, variables, maxSum):
    ...
    #pass
    # ### START CODE HERE ###
    sum_var_name = ('sum', name, 'aggregated')
    if len(variables) == 0:
        csp.add_variable(sum_var_name, [0])
        return sum_var_name
    csp.add_variable(sum_var_name, range(maxSum + 1))
    ...
    for i, var in enumerate(variables):
        aux_var_name = ('sum', name, i)
        var_domain = csp.values[var]
        aux_var_domain = [(s, var_val) for s in range(maxSum + 1) for var_val in var_domain]
        csp.add_variable(aux_var_name, aux_var_domain)
        
        csp.add_binary_factor(aux_var_name, var, lambda aux, v: aux[1] == v)
        
        if i == 0:
            csp.add_unary_factor(aux_var_name, lambda aux: aux[0] == 0)
        else:
            prev_aux_var_name = ('sum', name, i - 1)
            csp.add_binary_factor(aux_var_name, prev_aux_var_name, lambda aux, prev_aux: aux[0] == prev_aux[0] + prev_aux[1])
        
        sum_vars.append(aux_var_name)
    
    csp.add_binary_factor(sum_var_name, sum_vars[-1], lambda sum_var, last_aux_var: sum_var == last_aux_var[0] + last_aux_var[1])
...
class SchedulingCSPConstructor:
        ...
        for request in self.profile.requests:
            for quarter in self.profile.quarters:
                csp.add_unary_factor(
                    ...
                    or self.bulletin.courses[cid].is_offered_in(quarter),
                )
    ...
    def add_quarter_constraints(self, csp):
        ...
        # ### START CODE HERE ###
        for request in self.profile.requests:
            quarters_specified = request.quarters
            if quarters_specified:
                for quarter in self.profile.quarters:
                    if quarter not in quarters_specified:
                        for course_id in request.cids:
                            csp.add_unary_factor(
                                (request, quarter),
                                lambda cid: cid is None or cid != course_id
                            )
        # ### END CODE HERE ###
            ...
            for quarter_i, quarter in enumerate(self.profile.quarters):
                ...
                for pre_cid in req.prereqs:
                    ...
                    for pre_req in self.profile.requests:
                        ...
                        prereq_vars = [
                            (pre_req, q)
                            for i, q in enumerate(self.profile.quarters)
                            if i < quarter_i
                        ]
                        v = (req, quarter)
        ...
        for quarter in self.profile.quarters:
            ...
            for request in self.profile.requests:
                for course_id in request.cids:
                    ...
                    # Get the minimum and maximum units for the current course from the bulletin
                    min_units = self.bulletin.courses[course_id].minUnits
                    max_units = self.bulletin.courses[course_id].maxUnits
                    # Add variable for the current course in the current quarter
                    csp.add_variable(course_var, list(range(min_units, max_units + 1)) + [0]) 
                    course_vars.append(course_var)
                    csp.add_binary_factor((request, quarter), course_var, lambda req_cid, course_units: course_units > 0 if req_cid == course_id else course_units == 0)
            quarter_sum_var = get_sum_variable(csp, f'sum_units_{quarter}', course_vars, self.profile.maxUnits)
            csp.add_unary_factor(quarter_sum_var, lambda total_units: self.profile.minUnits <= total_units <= self.profile.maxUnits)
        # ### END CODE HERE ###
