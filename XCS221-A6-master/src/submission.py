...
class ExactInference(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        for row in range(numRows):
            for col in range(numCols):
                carX, carY = util.colToX(col), util.rowToY(row)
                trueDist = math.sqrt((carX - agentX) ** 2 + (carY - agentY) ** 2)
                prob = util.pdf(trueDist, Const.SONAR_STD, observedDist)
    ...
    def elapseTime(self):
        ...
        for (oldTile, newTile), transProb in self.transProb.items():
            ...
            newRow, newCol = newTile[0], newTile[1]
            newBelief.addProb(newRow, newCol, newProb)
...
class ParticleFilter(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        # ### START CODE HERE ###
        for tile in self.particles:
            trueDist = math.sqrt((agentX - util.colToX(tile[1])) ** 2 + (agentY - util.rowToY(tile[0])) ** 2)
            weight = util.pdf(trueDist, Const.SONAR_STD, observedDist)
            self.particles[tile] *= weight

        newParticles = collections.defaultdict(int)
        for _ in range(self.NUM_PARTICLES):
            newTile = util.weightedRandomChoice(self.particles)
